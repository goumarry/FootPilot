...
export default function BottomNav() {
  ...
  const itemsByRole: Record<string, Array<{ to: string; icon: React.ElementType; label: string; end?: boolean }>> = {
    ADMIN: [
      { to: '/admin', icon: LayoutDashboard, label: 'Admin', end: true },
      { to: '/admin/membres', icon: User, label: 'Membres' },
    ],
    GESTIONNAIRE: [
    ...
    ENTRAINEUR: [
      ...
      { to: '/dashboard/joueurs', icon: User, label: 'Joueurs' },
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-slate-900/90 backdrop-blur-md border-t border-slate-700/40">
      <div className="flex">
        {items.map((item) => {
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  <span className={clsx('text-[10px] font-semibold', isActive && 'text-violet-400')}>
                    {item.label}
                  </span>
                </>
              )}
